    ...
    for s in env.states:
        ...
        ax.add_patch(
            matplotlib.patches.Rectangle(
                s, 1, 1, edgecolor="k", facecolor=scalarMap.to_rgba(value)
            )
        )
        ax.annotate(
            f"{value:.2f}",
    ...
    for s_i, s in enumerate(env.states):
        ...
        ax.add_patch(
            matplotlib.patches.Polygon(
                [s_upper_left, s_centered, s_upper_right],
                edgecolor="k",
        ...
        ax.add_patch(
            matplotlib.patches.Polygon(
                [s_upper_right, s_centered, s_bottom_right],
                edgecolor="k",
...
def plot_policy(env, policy, ax=None):
    ...
    for s_idx, s in enumerate(env.states):
        ax.add_patch(
            matplotlib.patches.Rectangle(s, 1, 1, edgecolor="k", facecolor="w")
        )
        ...
        # Plot policy up
        ax.annotate(
            f"{policy[s_idx, 0]:.1f}".lstrip("0"),
            [s_centered[0], s_centered[1] + delta],
            c="k",
            fontsize=15,
            ha="center",
            va="center",
        ...
        # Plot policy right
        ax.annotate(
            f"{policy[s_idx, 1]:.1f}".lstrip("0"),
            [s_centered[0] + delta, s_centered[1]],
            c="k",
            fontsize=15,
            ha="center",
            va="center",
...
def plot_s_pmf(env, s, a, ax=None):
    ...
    for s_ in env.states:
        ax.add_patch(
            matplotlib.patches.Rectangle(s_, 1, 1, edgecolor="k", facecolor="w")
        )
    ax.axis("off")
    ax.relim()
    ax.autoscale_view()
    ...
    if env.terminal_table[env.states.index(s)] == 1:
        ax.add_patch(
            matplotlib.patches.Rectangle(s, 1, 1, edgecolor="k", facecolor="gray")
        )
        ax.annotate(
            "Terminal",
            [s[0] + 0.5, s[1] + 0.5],
    ...
    for s_prime_idx, prob in zip(s_prime_idxs, s_pmf):
        ...
        ax.add_patch(
            matplotlib.patches.Rectangle(
                s_prime, 1, 1, edgecolor="k", facecolor=scalarMap.to_rgba(prob)
            )
        )
...
def plot_all_states(env, ax):
    ...
    for i, s_ in enumerate(env.states):
        ...
        ax.add_patch(
            matplotlib.patches.Rectangle(s_, 1, 1, edgecolor="k", facecolor=facecolor)
        )
    ax.axis("off")
    ax.relim()
    ax.autoscale_view()
...
def plot_agent(s, ax, alpha=1.0):
    ...
    ax.add_patch(
        matplotlib.patches.Circle(
            s_centered, 0.08, edgecolor="k", facecolor="k", alpha=alpha
        )
    )
...
def plot_env_agent_and_policy_at_state(env, s, policy, ax):
    ...
    ax.annotate(
        f"{policy[s_idx, 0]:.1f}".lstrip("0"),
        [s_centered[0], s_centered[1] + delta_text],
        c="k",
        fontsize=15,
        ha="center",
        va="center",
    ...
    ax.annotate(
        f"{policy[s_idx, 1]:.1f}".lstrip("0"),
        [s_centered[0] + delta_text, s_centered[1]],
        c="k",
        fontsize=15,
        ha="center",
        va="center",
