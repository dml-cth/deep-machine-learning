...
def parse_cell_ids(nb_json_data):
    ...
    for cell in nb_json_data["cells"]:
        if (
            "metadata" in cell
            and "nbgrader" in cell["metadata"]
            and "grade_id" in cell["metadata"]["nbgrader"]
        ):
...
def check_notebook_uptodate_and_not_corrupted(nb_dirname, nb_fname):
    ...
    for expected_nb_fname in nb_fname_candidates:
        url_candidates.append(
            "http://raw.githubusercontent.com/dml-cth/deep-machine-learning/{commit}/home-assignments/{assignment_name}/{expected_nb_fname}".format(
    ...
    nbr_matching_cells = [
        len(curr_cell_ids & ref_cell_ids) for ref_cell_ids in ref_cell_id_candidates
    ]
